...
<Accordion title="Example File">
  ``` 
  boto3==1.20.24 boto3-stubs[s3,sts,sns,ses,logs] kubernetes awscli==1.22.24
  ```
</Accordion>

<Accordion title="Generated Docker Commands">
  ```Dockerfile 
  copy requirements.txt /opt/latch/requirements.txt run pip
  install --requirement /opt/latch/requirements.txt 
  ```
</Accordion>
